...
pygame.display.set_caption("Chess Game")  # We’ll dynamically update this later
...
"""
# Old code: loading images from script_dir only
...
def draw_board(selected_square=None, hover_square=None):
    ...
    for row in range(8):
        for col in range(8):
            ...
            # Convert (row, col) to top-left corner
    ...
    if selected_square is not None:
        ...
        rect = pygame.Rect(col * SQUARE_SIZE, row * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE)
        pygame.draw.rect(screen, HIGHLIGHT_RGB, rect, 4)  # border 4px
    ...
    if hover_square is not None:
        ...
        rect = pygame.Rect(col * SQUARE_SIZE, row * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE)
        pygame.draw.rect(screen, HIGHLIGHT_RGB, rect, 2)  # thinner border
...
def receive_move_from_server():
    ...
    # Blocks until a move is received, or None if disconnected.
...
def listen_for_opponent_moves(board):
    ...
    while True:
        ...
        # Old code: Directly push move
        # move = chess.Move.from_uci(move_uci)
        # board.push(move)
        
        # New code: Add to queue
...
def main():
    ...
    # Old code: just setup socket without waiting
    # if not player_vs_ai:
    #     setup_socket()
    #     threading.Thread(target=listen_for_opponent_moves, args=(board,), daemon=True).start()
    
    # New code: If PvP, setup network and wait for color assignment
    if not player_vs_ai:
        setup_socket()
        threading.Thread(target=listen_for_opponent_moves, args=(board,), daemon=True).start()
        # Wait until we know if we're white or black
    ...
    while running:
        ...
        if opponent_disconnected:
            # Show a message and let the user exit or return to menu
        ...
        if board.is_game_over():
            ...
            # If user picks new game, re-run main()
            if result == "new_game":
                main()
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if selected_square is None:
                    # If there's a piece on that square owned by the current player
        ...
        # Draw board + pieces with highlight
...
# (Nothing here)
"""
incoming_moves = queue.Queue()  # New code: Create a queue to store incoming opponent moves
...
HIGHLIGHT_RGB = (255, 255, 0)  # Yellow highlight color for hover or selected
screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Chess Game")  # We’ll update this title dynamically
...
"""
# Old code: Loading images from the script directory only
# script_dir = os.path.dirname(os.path.abspath(__file__))
# for piece, filename in PIECE_IMAGE_MAP.items():
#     path = os.path.join(script_dir, filename)
#     if os.path.exists(path):
